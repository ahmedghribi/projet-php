  ...
  <div class="table-responsive">
    <table class="table table-hover">
      ...
      foreach ($clients as $resultat) {
        $typeclients1=$cnx->query("select remise from typeclient where id='".$resultat->type."'")->fetch(PDO::FETCH_OBJ);
       
        $typeclients=$cnx->query("select nom from typeclient where id='".$resultat->type."'")->fetch(PDO::FETCH_OBJ);
        echo '<tr>';
        echo '<td>' . $resultat->nom . '</td>';
        echo '<td>' . $resultat->tel . '</td>';
        echo '<td>' . $resultat->email . '</td>';
        echo '<td>' . $typeclients->nom . '</td>';
        echo '<td>' . $typeclients1->remise . ' %</td>';
        echo '<td>';
